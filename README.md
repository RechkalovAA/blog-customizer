...
Важные файлы:
- `src/index.tsx` — точка входа приложения, компонент `App`
- `src/constants/articleProps.ts` — файл с типами, константами и опциями настроек
...
Взаимодействие между компонентами обеспечивается через пропсы и обработчики событий. Состояние формы и примененное состояние разделены для реализации требования "настройки не применяются сразу".

### Базовые компоненты

#### Компонент App
Главный компонент приложения, отвечает за композицию и управление состоянием сайдбара.

**Назначение:** Координация работы всех компонентов, применение CSS-переменных к статье.

**Расположение:** `src/index.tsx`

**Состояние:**
- `isSidebarOpen: boolean` — состояние открытия/закрытия сайдбара

**Используемые хуки:**
- `useArticleParams()` — кастомный хук для управления состоянием статьи

**Методы:**
- `handleToggleSidebar()` — переключает состояние открытия сайдбара
...
- `--bg-color` — цвет фона
- `--image-width` — ширина изображения (вычисляется динамически: 1600px для широкого режима, 948px для узкого)
...
- `isOpen: boolean` — состояние открытия/закрытия сайдбара
- `onToggle: () => void` — обработчик переключения состояния сайдбара
